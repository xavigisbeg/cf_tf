...
{                        // const because we are just reading it, & to pass by reference
    //geometry_msgs::Pose::_position_type CFposep[4][3];
    //geometry_msgs::Pose::_orientation_type CFposeo[4][4];
    ...
    // We have a world
        ...
        if (msg.detections.empty() != true)
        {
            ...
            {                               // Checks if a marker has been detected
                try
                {
                    /*switch (msg.detections[i].id){                                          // We do this so that we know that we can address the pose to the right marker. Will be replaced by the radio choice
                    ...
                    /*CFposep[msg.detections[i].id][0] = msg.detections[i].pose.pose.position.x;
                    CFposep[msg.detections[i].id][1] = msg.detections[i].pose.pose.position.y;
                    CFposep[msg.detections[i].id][2] = msg.detections[i].pose.pose.position.z;
                    CFposeo[msg.detections[i].id][0] = msg.detections[i].pose.pose.orientation.x;
                    CFposeo[msg.detections[i].id][1] = msg.detections[i].pose.pose.orientation.y;
                    CFposeo[msg.detections[i].id][2] = msg.detections[i].pose.pose.orientation.z;
                    CFposeo[msg.detections[i].id][3] = msg.detections[i].pose.pose.orientation.w;
                    ...
                    transformcf[i].setOrigin(tf::Vector3(CFposep[i][0], CFposep[i][1], CFposep[i][2]));
                    transformcf[i].setRotation(tf::Quaternion(CFposeo[i][0], CFpose[i][1], CFpose[i][2], CFpose[i][3]));
                    br.sendTransform(tf::StampedTransform(transformcf[i], ros::Time::now(), "world" , "CF" + sprintf(i)));  */
